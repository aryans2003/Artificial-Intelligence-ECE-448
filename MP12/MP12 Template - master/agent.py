...
class Agent:
    ...
    def act(self, environment, points, dead):
        ...
        # 1) first step: we have to use the generate_state to discretize the env into state env (s_prime) -- DONE
        s_prime = self.generate_state(environment)

        # TODO - MP12: write your function here

        # 2) if we are in training mode...
        if self._train:
            # check if the previous state-action pair exists
            if self.s is not None and self.a is not None:
                # if so... increment the visit count in the N-table
                self.update_n(self.s, self.a)
                # and update the Q-table accordingly
                # we first need to calculate with which reward we update the Q-table...
                # if our agent is dead --> -1 (to indicate death)
                if dead:
                    q_reward = -1
                # else if alive... 
                # but prev state points < new points
                elif self.points < points:
                    # set the prev state points to the new points
                    self.points = points
                    q_reward = 1 # and put q_reward = -1
                # else if alive but prev state points NOT < new points...
                else:
                    q_reward = -0.1 # default reward for every step
                # update Q-table for all based on q_reward
                # self.s, self.a stay the same
                # using new state env s_prime
                self.update_q(self.s, self.a, q_reward, s_prime)
                # if dead we also have a NEW condition...
                if dead:
                    # after dying, we want to reset snake's state
                    self.reset()
                    # return any action upon death... I'll just use the previous state action
                    return self.a

        # 3) choosing the best action      
        # set a temporary value for to hold our 'best' value
        # keep it at the lowest # possible now for comparison purposes
        best_value = -float('inf') 
        # for BOTH training and testing modes...
        # we loop thru each possible action given in self.actions
        for action in self.actions:
            # grabbing index of the current action
            action_idx = self.actions.index(action)
            # getting the Q-value action value given discretized state and action index
            action_value = self.Q[s_prime][action_idx]

            # if the action has been taking fewer times than the exploration threshold...
            if self.Ne > self.N[s_prime][action_idx]:
                # assign exploration value as +1 (high)
                action_value = 1

            # if the best action has a higher or equal value....
            if action_value >= best_value:
                # swap the best value with the best action value
                best_value = action_value
                # swap chosen action with the new action
                chosen_action = action

        # update to the new discretized state
        self.s = s_prime
        
        # update for both training AND testing we change self.a to the new chosen action
        self.a = chosen_action
        
        # return the new chosen action (stored in self.a)
        return self.a
