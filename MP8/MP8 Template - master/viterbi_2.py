...
# we want to copy all the functions over from viterbi_1
# training should be changed to implement better smoothing techniques for unseen and rare words
# stepforward is the same
# viterbi_2 is the same as viterbi_1
...
def training(sentences):
    ...
    hapax_count = Counter() # dict to count occurrences for hapax words
    hapax_tag_frequencies = Counter() # dict to count tag frequencies for hapax words
    hapax_tag_probabilities = {} # init dict to hold hapax tag probabilities
    
    # For viterbi_2 training implementation, we want to add hapax probabilities for unseen words
    ...
    # 2) computing emission and transition probabilities together AND NOW hapax words
    # loop thru sentences
    for sentence in sentences:
        for i, (word, tag) in enumerate(sentence):
            ...
            # for the hapax probabilities...
            # count word ocurrences to identify hapax words
            if hapax_count[word] == 0: # first occurrence of word
                hapax_tag_frequencies[tag] += 1 # increment its occurrence in freq counter
            elif hapax_count[word] == 1: # we find another occurrence, it is no longer a hapax
                # note with this case, it doesnt matter > 1 because everytime it's 2 we just - 1
                hapax_tag_frequencies[tag] -= 1 # remove occurrence from hapax counter
            hapax_count[word] += 1 # increment word count of all words so we can check if word comes again
        ...
        # make sure tags have at least one occurrence in hapax_tag_freq (one for unseen prob) --> Laplace smoothing
        for tag in tag_freq:
                # if tag is zero...
                if hapax_tag_frequencies[tag] == 0:
                        # make it one
                        hapax_tag_frequencies[tag] = 1

        # Calculate hapax tag probabilities as sum of values
        total_hapax = sum(hapax_tag_frequencies.values())
        # loop thru tag and tag's count in hapax_tag_freq
        for tag, count in hapax_tag_frequencies.items():
                # check if total_hapax is 0
                if total_hapax > 0:
                        probability = count / total_hapax # if not, calc prob of hapax tag
                else: # if 0...
                        probability = 0 # probability is just zero
                # assign probability to right tag in the dict
                hapax_tag_probabilities[tag] = probability
                
    # 3) calculating emission probabilities here
    # DIFFERENT FOR MP2 --> dynamically changing
    # we want to first loop thru tags in tag_freq
    for tag in tag_freq:
            # then loop thru each word in the unique word's database by its tag
            for word in unique_words[tag]:
                    count = word_tag[(word, tag)] # get count of word, tag pair
                    # calculate emit_prob using the NEW formula
                    # (count + hapax_epsilon) / (tag_freq[tag] + old_epsilon * (len(unique_words[tag] + 1))
                    hapax_epsilon = emit_epsilon * hapax_tag_probabilities[tag] # new epsilon factoring in the hapax prob for the tag
                    emit_prob[tag][word] = (count + hapax_epsilon) / (tag_freq[tag] + emit_epsilon * (len(unique_words[tag]) + 1))
            # for UNK words in each tag
            # new formula --> (emit_epsilon * hapax_tag_probabilities[tag]) / (tag_freq[tag] + emit_epsilon * (len(unique_words[tag]) + 1))
            emit_prob[tag]['UNKNOWN'] =  (emit_epsilon * hapax_tag_probabilities[tag]) / (tag_freq[tag] + emit_epsilon * (len(unique_words[tag]) + 1))
